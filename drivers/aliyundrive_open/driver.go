...
func (d *AliyundriveOpen) Init(ctx context.Context) error {
	...
	d.DriveId = utils.Json.Get(res, "default_drive_id").ToString()
        ...
        if (d.resource_drive_id != "") && (d.RorB == "r") {
                d.DriveId = d.resource_drive_id
                fmt.Println("资源库ID:", d.resource_drive_id)
        }
        if (d.backup_drive_id != "") && (d.RorB == "b")  {
                d.DriveId = d.backup_drive_id
                fmt.Println("备份库ID:", d.backup_drive_id)
        }	
	return nil
}
