...
func (d *AliyundriveShare2Open) Init(ctx context.Context) error {
	...
	if ! strings.EqualFold(conf.Conf.Opentoken_auth_url, "https://openapi.alipan.com/oauth/access_token") {
    	d.cron2 = cron.NewCron(time.Minute * 3)
	} else {
		d.cron2 = cron.NewCron(time.Minute * 3)	
	}
    d.cron2.Do(func() {
...
func (d *AliyundriveShare2Open) ensureValidToken(ctx context.Context) error {
    ...
    if d.MyAliDriveId == "" {
		...
		base.MyAliDriveId = d.MyAliDriveId
    }
    return nil
}
...
func (d *AliyundriveShare2Open) Link(ctx context.Context, file model.Obj, args model.LinkArgs) (*model.Link, error) {
	...
	if err == nil {
		if  DownloadUrl != "http://img.xiaoya.pro/abnormal.png" {
...
func (d *AliyundriveShare2Open) GetmyLink(ctx context.Context, file_id, old_file_id string, file_name string) (string, error) {
    ...
    for {
        ...
        if err == nil {
            ...
            d.Hash_dict[old_file_id] = strings.ToUpper(utils.Json.Get(res, "content_hash").ToString())
            fmt.Println("文件: ",file_name,"  新增下载直链: ", d.DownloadUrl_dict[file_id])
			// fetch CDN link --begin
			res, err1 := d.requestOpenCDN("https://bj29.api.aliyunpds.com/v2/file/get_download_url", http.MethodPost, func(req *resty.Request) {
				req.SetBody(base.Json{
                    "drive_id":   d.MyAliDriveId,
                    "file_id":    file_id,
                    "expire_sec": 14300,
			...
			if err1 != nil {
				fmt.Println("获取CDN错误: ",err1)
			}
			if err1 == nil {
				if utils.Json.Get(res, "cdn_url").ToString() != "" {
					d.DownloadUrl_dict[file_id] = utils.Json.Get(res, "cdn_url").ToString()
					fmt.Println("文件: ",file_name,"  新增CDN直链: ", d.DownloadUrl_dict[file_id])
				}
			}
			// fetch CDN link --end
...
// GetHash returns the file hash from Hash_dict
