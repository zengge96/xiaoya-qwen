...
func (d *AliyundriveShare2Pan115) Init(ctx context.Context) error {
    ...
    if ! strings.EqualFold(conf.Conf.Opentoken_auth_url, "https://openapi.alipan.com/oauth/access_token") {
        d.cron2 = cron.NewCron(time.Hour * 4)
    } else {
	...
	d.cron3 = cron.NewCron(time.Minute * 90)
	d.cron3.Do(func() {
		if len(d.pickCodeMap) > 0 {
...
func (d *AliyundriveShare2Pan115) ensureValidToken(ctx context.Context) error {
...
func (d *AliyundriveShare2Pan115) Link(ctx context.Context, file model.Obj, args model.LinkArgs) (*model.Link, error) {
	...
	go func() {
        time.Sleep(2 * time.Second)
		if files, err := d.client.List(d.DirId); err == nil && strings.EqualFold(d.PurgePan115Temp,"true") {
			for i := 0; i < len(*files); i++ {
				file := (*files)[i]
				if file.Name == file_name && strings.ToUpper(file.Sha1) == fullHash{
					d.client.Delete(file.FileID)
					fmt.Printf("\033[0;32m%s%s%s\033[0m\n",time.Now().Format("01-02-2006 15:04:05"),"转存115文件成功删除：",file.FileID)
				}
			}
		}
		if strings.EqualFold(d.PurgeAliTemp,"true"){
